...
coverage_percent = st.slider("Select Insurance Coverage Percentage", 0, 100, 70)
insurance_rows = []
for drug in drug_selections:
    generic_price = drug_data[drug]["generic_price"]
    brand_price = drug_data[drug]["brand_price"]
    generic_out_of_pocket = round(generic_price * (1 - coverage_percent / 100), 2)
    brand_out_of_pocket = round(brand_price * (1 - coverage_percent / 100), 2)
    insurance_rows.append({
        "Drug": drug,
        "Generic Price ($)": generic_price,
        "Brand Price ($)": brand_price,
        "Generic Out-of-Pocket ($)": generic_out_of_pocket,
        "Brand Out-of-Pocket ($)": brand_out_of_pocket
    })
df_insurance = pd.DataFrame(insurance_rows)
df_insurance.insert(0, "Sr. No.", range(1, len(df_insurance) + 1))
