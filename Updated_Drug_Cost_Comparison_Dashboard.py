...
drug_labels = [f"{drug} ({drug_data[drug]['condition']})" for drug in drug_data]
drug_map = {f"{drug} ({drug_data[drug]['condition']})": drug for drug in drug_data}
selected_labels = st.sidebar.multiselect("Select Drugs", drug_labels)
selected_drugs = [drug_map[label] for label in selected_labels]
...
# Insurance Coverage Estimator for Patients
if drug_selections and role == "Patient":
    st.subheader("🧾 Insurance Coverage Estimator")
    coverage_rows = []
    for drug in drug_selections:
        generic_price = drug_data[drug]["generic_price"]
        brand_price = drug_data[drug]["brand_price"]
        coverage_percent = random.choice([60, 70, 80, 90])
        generic_covered = round(generic_price * coverage_percent / 100, 2)
        brand_covered = round(brand_price * coverage_percent / 100, 2)
