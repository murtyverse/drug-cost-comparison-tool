...
# Sidebar with elegant layout
st.sidebar.markdown("## 🧭 Dashboard Navigation")
st.sidebar.markdown("Use the options below to explore the dashboard.")
st.sidebar.markdown("---")
selected_sections = st.sidebar.multiselect(
    "📌 Select Sections to View",
    ["💊 Price Comparison", "📊 Generic vs Brand", "💰 Savings Estimator", "🧠 Therapeutic Alternatives"],
    default=["💊 Price Comparison", "📊 Generic vs Brand", "💰 Savings Estimator", "🧠 Therapeutic Alternatives"]
)
...
# Price Comparison
if "💊 Price Comparison" in selected_sections and drug_selections:
    ...
    df_comparison.index = range(1, len(df_comparison) + 1)
    df_comparison.reset_index(inplace=True)
    df_comparison.rename(columns={"index": "Sr. No."}, inplace=True)
    st.dataframe(df_comparison)

# Generic vs Brand Visualization
if "📊 Generic vs Brand" in selected_sections and drug_selections:
...
# Savings Estimator
if "💰 Savings Estimator" in selected_sections and drug_selections:
    ...
    df_savings.index = range(1, len(df_savings) + 1)
    df_savings.reset_index(inplace=True)
    df_savings.rename(columns={"index": "Sr. No."}, inplace=True)
    st.dataframe(df_savings)

# Therapeutic Alternatives
if "🧠 Therapeutic Alternatives" in selected_sections and drug_selections:
    ...
    for drug in drug_selections:
        ...
        df_alt.index = range(1, len(df_alt) + 1)
        df_alt.reset_index(inplace=True)
        df_alt.rename(columns={"index": "Sr. No."}, inplace=True)
        st.dataframe(df_alt)
