...
drug_data = {
    ...
    "Amlodipine": {"condition": "High Blood Pressure", "dosages": ["5mg", "10mg"], "generic_price": 11, "brand_price": 32}
}
...
drug_options = [f"{drug} ({drug_data[drug]['condition']})" for drug in drug_data]
selected_drugs_display = st.sidebar.multiselect("Select Drugs", drug_options)

# Map display names back to actual drug names
selected_drugs = [drug.split(" (")[0] for drug in selected_drugs_display]

drug_selections = {}
for drug in selected_drugs:
    dosage = st.sidebar.selectbox(f"Select Dosage for {drug} ({drug_data[drug]['condition']})", drug_data[drug]["dosages"], key=drug)
    ...
    for drug, dosage in drug_selections.items():
        for pharmacy in pharmacies:
            ...
            comparison_rows.append({
                "Drug (Condition)": f"{drug} ({drug_data[drug]['condition']})",
                "Dosage": dosage,
                "Pharmacy": pharmacy,
